...
export function home(notes, usingDatabase) {
  const storage = usingDatabase
    ? 'Saving to the database'
    : 'Saving in memory, resets on restart';

  const items = notes.length
    ? notes.map((n) => `  <li>${escapeHtml(n.body)}<time>${new Date(n.created_at).toISOString().replace('T', ' ').slice(0, 16)}</time></li>`).join('\n')
    : '  <li style="opacity:.6">Nothing here yet. Add the first note.</li>';

  return page('Node starter', `<h1>It works.</h1>
<p class="sub">Your Node.js app is running. <span class="badge">${escapeHtml(storage)}</span></p>
...
<footer>
  Edit <code>src/server.js</code> to change this page.<br>
...
export function serverError() {
  return page('Something broke', '<h1>500</h1>\n<p>Something went wrong on our end. Check the logs in the Actions tab.</p>');
}
